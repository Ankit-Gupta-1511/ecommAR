...
export function setupT3d(virtualMirror) {
  "use strict";

  var t3d    = virtualMirror.drawing3d.t3d;  // ThreeJS namespace.
  var dom    = virtualMirror.dom;      // ... e.g. the DOM handling.

  if(!t3d) {
    virtualMirror.drawing3d.t3d = {};
  }
  ...
  t3d.setup = function(canvas) {
    ...
    t3d.renderWidth   = 640;
    t3d.renderHeight  = 480;

    t3d.updateLayout(t3d.stage.width, t3d.stage.height);
  };
  ...
  t3d.update = function(index, face, show) {
    ...
    if(!t3d.renderwidth) t3d.renderWidth = 640;

    if(!t3d.renderHeight) t3d.renderHeight = 480;
  ...
  if(t3d.setup && !t3d.stage) {
    t3d.setup(document.getElementById("_t3d"));
  }
};
