	...
	function onExampleChosen(data) {
		...
		if(url) {
			if(typeof url === "string") {
				virtualMirror.loader.loadExample([url], onExampleLoaded);
			} else {
				virtualMirror.loader.loadExample(url, onExampleLoaded);
			}
		} else {
			if(data.index >= 0) {
				virtualMirror.gui.exampleChooser.setValuesFromJSON({ "_example": data.index + 1});
			}
		}
	}

	if(!virtualMirror.gui.exampleChooser) {

		QuickSettings.useExtStyleSheet();

		virtualMirror.gui.exampleChooser = QuickSettings.create(
			2, 2, "Example Chooser", virtualMirror.dom.createDiv("_settingsRight"))
			.setWidth(250)
